	...
	<form action="" id="manage-topic">
			...
			<div class="col-md-8">
				...
				<select name="category_ids[]" id="category_ids" multiple="multiple" class="custom-select select2">
					...
					while ($row = $tag->fetch_assoc()) :
						$selected = isset($category_ids) && in_array($row['id'], explode(",", $category_ids)) ? "selected" : '';
					?>
		...
		<div class="row form-group">
			<div class="col-md-12">
				...
				<textarea name="content" class="text-jqte"><?php echo isset($content) ? htmlspecialchars($content) : '' ?></textarea>
			</div>
		</div>
	</form>
</div>
